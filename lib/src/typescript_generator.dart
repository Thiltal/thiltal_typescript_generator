...
//      fields.add("// isSynthetic ${field.isSynthetic}");
      ...
      for (var annotation in field.metadata) {
        ...
        if (annotation.toString().contains("TypescriptReplaceType")) {
          if (annotation.computeConstantValue() != null) {
            if (annotation.computeConstantValue().type.name == 'TypescriptReplaceType') {
              type = annotation.computeConstantValue().getField('newType').toStringValue();
            }
          }
        }
      }
      if (field != null) {
        fields.add('${field.name}${isOptional ? "?" : ""}: ${type != null ? type : _resolveType(field.type)};');
      }
    });
